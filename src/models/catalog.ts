...
export function createCatalog(): CatalogItem[] {
  ...
  for (const [role, name] of [['raider', 'MARCHER'], ['diver', 'SWOOPER'], ['flanker', 'WEAVER']] as const) items.push({ title: `ALIEN ${name}`, description: 'Invaders mode enemy. Breaks into marches, swoops and pincer attacks. Shoot incoming fire to charge blast.', create: () => createInvaderModel(role), scale: 2.2, cameraZ: 62 });
  items.push(
  ...
  const cargo: Array<[CargoType, string]> = [
    ...
    ['weaponCore', 'Improves the equipped weapon; capped cores award points in Invaders or credits in trading modes.'], ['shieldCell', 'Partially repairs hull and restores some shield points. Collect it during combat.'], ['rescuePod', 'Protected mission cargo. Deliver to the station.']
  ];

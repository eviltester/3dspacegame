...
const vitals = (health: number) => ({ health, shield: 0, damage: 0 });

it.each(['shield', 'repair'] as const)('Smuggler %s pickup repairs its single craft and reports the correct amount', kind => {
  const root = new THREE.Group(), drops = new SkiffRepairDrops(root, () => 0, true), point = new THREE.Vector3();
  drops.release(kind === 'shield' ? 'rock' : 'turret', point);
  expect(drops.step(0, point, point, { health: 1, shield: 10, damage: 80 })).toEqual({ health: 1, shield: kind === 'shield' ? 30 : 100, damage: kind === 'shield' ? 60 : 0 });
  expect(drops.notice).toBe(kind === 'shield' ? 'SHIELD +20 / DAMAGE REPAIRED 20' : 'REPAIR PICKUP / SKIFF FULL');
});
...
it.each([['rock', 'shield', 2], ['turret', 'repair', 3]] as const)('collecting %s drop repairs once, even across a fast swept crossing', (source, kind, expected) => {
  ...
  expect(drops.step(0.1, new THREE.Vector3(), new THREE.Vector3(0, 0, -100), vitals(1)).health).toBe(expected);
  expect(drops.collected).toBe(1); expect(drops.notice).toContain(kind.toUpperCase()); expect(root.children).toHaveLength(0);
  expect(drops.step(0.1, new THREE.Vector3(), new THREE.Vector3(0, 0, -100), vitals(expected)).health).toBe(expected);
...
it('only attracts near pickups, animates them, and never resurrects a destroyed skiff', () => {
  ...
  expect(drops.step(0.01, origin, origin, vitals(1)).health).toBe(1);
  ...
  expect(drops.step(0.1, origin, origin, vitals(0)).health).toBe(0); expect(drops.collected).toBe(0);
  drops.release('rock', origin); expect(drops.step(0, origin, origin, vitals(3)).health).toBe(3); expect(drops.collected).toBeGreaterThan(0);
});
it('bounds active drops and disposes missed or expired models', () => {
  ...
  drops.step(20, origin, origin, vitals(3)); expect(root.children).toHaveLength(0); expect(dispose).toHaveBeenCalledOnce();
  drops.release('rock', new THREE.Vector3(50, 0, 45)); drops.step(0, origin, origin, vitals(3));

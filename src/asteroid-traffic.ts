/** One-way traffic mixed into later asteroid fields; hostile patrols in Smuggler Run. */
...
import { BeltFire } from './belt-fire';
...
export class AsteroidTraffic {
  ...
  readonly fire: BeltFire;
  private readonly slots = new Set<number>();
  private readonly random: Random;
  private readonly speed: number;
  private readonly explosions: ShipExplosions;
  constructor(private root: THREE.Group, seed: number, difficulty: number, private hostile = false, private sounds = new SoundEvents()) {
    ...
    this.fire = new BeltFire(root, profile.level, sounds);
  ...
  step(dt: number, time: number, previous: THREE.Vector3, player: THREE.Vector3, playerAlive = true) {
    ...
    for (const ship of this.ships) {
      ...
      if (!ship.active && ahead > 0 && ahead < 700 && active < MAX_BELT_TRAFFIC) {
        ...
        if (playerAlive) this.sounds.cue('patrolApproach');
      }
      ...
      if (playerAlive && sweptHit(previous, player, ship.previous, ship.object.position, ship.radius + 2) !== null) {
        impacts++; this.destroy(ship);
      }
      if (ship.object.position.z > player.z + 40) ship.used = true;
      if (ship.used) ship.object.visible = false;
    }
    const fire = this.fire.step(dt, time, this.hostile && playerAlive ? this.ships : [], previous, player, playerAlive);
    return { ...fire, impacts, notice: fire.notice || notice };
  }
  ...
  get contacts(): RadarContact[] {
    return [...this.ships.filter(ship => !ship.used).map(ship => ({ position: ship.object.position,
      color: ship.kind === 'pirate' ? '#ff4055' : '#75caff', glyph: 'ship' as const })),
    ...this.fire.shots.filter(shot => !shot.used).map(shot => ({ position: shot.object.position, color: shot.color, glyph: 'shot' as const }))];
  }
  get snapshot() {

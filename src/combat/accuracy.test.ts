...
describe('individual projectile accuracy', () => {
  it('counts three Spread bolts with one hit as one hit, two misses and a ten-point cost', () => {
    const tracker = new ShotAccuracy(), stats = emptyAccuracy();
    for (let id = 1; id <= 3; id++) tracker.begin(id, stats);
    tracker.hit(2, stats);
    expect([1, 2, 3].reduce((cost, id) => cost + tracker.end(id, stats), 0)).toBe(10);
  ...
  it('counts each piercing bolt once despite several contacts or repeated callbacks', () => {
    const tracker = new ShotAccuracy(), stats = emptyAccuracy();
    tracker.begin(1, stats); tracker.begin(1, stats);
  ...
  it('settles in-flight misses once at wave clear, without charging successful bolts', () => {
    const tracker = new ShotAccuracy(), stats = emptyAccuracy();
    [1, 2, 3].forEach(id => tracker.begin(id, stats)); tracker.hit(1, stats);
    expect(tracker.finish(stats)).toBe(10); expect(tracker.finish(stats)).toBe(0);
  ...
  it('ignores untracked fire and cancels pending work without charging on reset', () => {
    ...
    tracker.begin(1, stats); tracker.clear(); expect(tracker.finish(stats)).toBe(0);
...
describe('wave accuracy checkpoints', () => {
  ...
  it('rolls back accuracy and score on death, continue and interrupted-wave resume', () => {
    ...
    expect(profile.checkpoints.invaders!.pilot.score).toBe(300);
    expect(run.accuracy.shots).toBe(3);
    loseLife(run); expect(run.pilot.score).toBe(300); expect(run.accuracy).toEqual(emptyAccuracy());

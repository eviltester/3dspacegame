...
export function canyonSpeed(progress: number, boost: boolean, difficulty = 1): number {
  return (48 + 96 * THREE.MathUtils.clamp(progress, 0, 1)) * (boost ? 1.5 : 1) * bonusProfile(difficulty).flightScale;
}
...
export class CanyonCourse {
  ...
  constructor(private root: THREE.Group, seed: number, difficulty = 1) {
    ...
    for (let i = 0; i < profile.canyonObstacles; i++) {
      ...
      if (i % 2 === 0) {
        ...
        const gun: Gun = { kind: 'turret', object, radius: 8, number: 0, used: false, cooldown: 0.3 + rng.range(0, 0.5), windup: 0, aim: new THREE.Vector3() };
      ...
      if (gun.windup > 0) {
        ...
        if (gun.windup === 0) {
          ...
          if (this.shots.length < CANYON_MAX_BOLTS) {
            const object = createBoltModel(0xff4055, 2.4, 14, 'pirate'); object.position.copy(gun.object.position);

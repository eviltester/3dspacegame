...
- Use the on-screen pause and centre-tilt buttons. The + / - buttons adjust free-flight throttle through stop into reverse. Boost is available in free flight and canyons; automatic courses hide the throttle buttons.
...
Smuggler Run alternates asteroid belts and canyon runs. Legs 1/2 use difficulty 1, legs 3/4 use difficulty 2, through difficulty 8 at legs 15/16; later legs generate new seeded courses at that maximum difficulty. Each leg uses a fresh runner with three hull points, all three tier-one weapons and a defensive blast. Canyon skiffs also start with 100 shield. Asteroid salvage/target units are worth 25 points; canyon gate/combat rewards and penalties count at their displayed values. Flight points survive crashes, missed exits, timeouts and saved restarts. Canyon haul stays as cargo until delivery, with 75 points per collected pickup at EXIT; undelivered haul is lost on a failed or restarted leg. Reaching EXIT also adds 1,000 delivery points, 100 per remaining hull point and 150 per difficulty above 1. A destroyed craft or missed exit costs a life and retries the leg. Every 5,000 points awards an extra life at flight settlement, capped at five; thresholds are consumed even at the cap. There is no safe bonus-exit button in this mode.

All four modes begin with three lives. Score accumulates across every life, including the final loss, and resets only for a new game or Continue. While lives remain, a destroyed combat ship automatically respawns with full hull and shields, keeping the current fight, equipment, cargo, score and wave accuracy. Three seconds of protection flash the visible ship blue; cockpit flight shows a blue shield outline and a lives-remaining message. Protection freezes during pause and restores normal colours when it expires. Failed main objectives and Smuggler courses restart automatically from their route/equipment checkpoint without rolling score back. Only zero lives opens the ten-second game-over screen; Continue immediately refills three lives and resets the score. Leaving preserves the available checkpoint. Each mode owns its checkpoint, best/continued records and a separate top-ten high-score screen; an ongoing run updates its entry instead of adding duplicate rows.

Successful Smuggler legs show the accumulated score, lives and next-level countdown for three seconds, then launch the next belt or canyon automatically. Canyon deliveries also show the haul conversion, such as `HAUL 3 x 75 = +225`. There are no mission menus between legs. Pause, lost focus and hidden tabs freeze the countdown. The initial launch still has a briefing; resuming a completed leg shows the short summary and its saved haul breakdown without repeating its payout.
...
Every canyon projectile that hits neither a target nor hostile fire costs 50 points. Spread counts each of its three bolts separately: one hit and two misses costs 100 points, minus the target reward. Lance's piercing hits count as one successful bolt. Blasts have no miss cost. Shot misses do not increase the gate-penalty counter or block charging by themselves.

Shot asteroids (including fragments) have a 1 in 15 chance of dropping a blue shield and a 1 in 30 chance of dropping a pink full repair. Shot canyon crates have the shield chance; shot canyon guns have the full-repair chance. Each destroyed object can release at most one repair pickup; a crate's separate haul roll can also succeed. Collecting a shield repairs one skiff hull point and restores 20 canyon shield. A full repair restores all three hull points and all 100 canyon shield. Neither exceeds capacity or revives a destroyed craft. Pickups appear as triangles on the radar and have a 20-unit collection magnet. Blasts and collisions do not release pickups. These rules apply to both optional sorties and Smuggler Run.

Canyon gun hits each remove 20 shield. A hit that empties the remaining shield does not also damage hull; subsequent unshielded hits each remove one hull point. Hitting the canyon side or a solid obstacle empties any remaining shield, or removes one hull point if shields were already zero. Continuous contact has a 1.2-second impact grace period so a single scrape cannot spend all hull in consecutive frames. Gun hits are counted individually, including simultaneous shots. Respawn protection ignores both kinds of damage. The final EXIT wall still ends a missed-exit flight.
...
From asteroid difficulty 3, some rock positions contain oncoming pirate and police ships: four ships initially, increasing to fourteen at difficulty 8. They approach faster than drifting rocks, weave gently, point their noses along their flight direction and never turn back after passing. At most three are moving toward the skiff at once. Arrivals are announced and faction-coloured ships appear on radar. Pirates can be destroyed for one target unit (25 Smuggler points) and break into spinning hull panels and sparks; police are friendly and immune to course weapons and blasts. Either ship can damage the skiff in a collision. These are passing encounters, not ships that chase or fire at the skiff.
